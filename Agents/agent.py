...
_HOLDINGS_RISK_INSTRUCTION = (
    ...
    "應該可以在對話紀錄中看到該次呼叫的結果：\n"
    "- 如果 get_holdings 回傳了非空的真實持股清單，以這份真實資料"
    "（含每筆的 stock_code 與 quantity，或已知的 market_value）作為"
    "投資組合權重計算的基礎，不要要求使用者重新輸入。\n"
    "- 如果 get_holdings 回傳空清單或錯誤，但使用者自己的訊息中已經"
    "說明了持股內容（例如「我有台積電 500 股、鴻海 300 股」），改用"
    "使用者說的內容（stock_code + quantity）作為分析基礎，可以在回覆"
    "中順帶提一句「以你提供的持股資訊估算」，不要暗示這是從資料庫查"
    "到的。\n"
...
pull_stock_agent = Agent(
    ...
    instruction=(
        ...
        "什麼」）★\n"
        "呼叫 get_holdings 取得使用者目前的真實持股清單（含即時市值與"
        "佔比），用白話方式呈現給使用者，例如總市值、各檔佔比、目前"
        "賺賠概況。這個工具會自動識別目前對話的使用者身份，不需要也"
        "不應該詢問使用者的 LINE ID 或帳號。\n\n"
        "如果工具回傳「目前對話沒有可用的使用者身份」的錯誤，誠實告知"
        "使用者這個環境暫時無法查詢真實持股，不要編造資料。如果使用者"
        "目前沒有任何持股，也直接誠實告知，並可以引導使用者「跟我說你"
        "買了什麼」來開始記錄。\n\n"
        ...
        "會不會影響我的持股風險」★\n"
        "一律先呼叫 get_holdings（不要先反問使用者），再依照結果分三種"
        "情況處理：\n"
        "1. get_holdings 回傳非空的真實持股清單 -> 呼叫 transfer_to_"
        "agent(\"stock_analysis_agent_via_holdings\") 轉交，讓它接手"
        "用這份真實資料做風險分析。不要自己複述持股內容或分析結果，"
        "直接轉交即可。\n"
        "2. get_holdings 回傳空清單或錯誤，但使用者這則訊息裡「自己已經"
        "說明了」持股內容（例如「我有台積電 500 股」）-> 同樣轉交"
        "transfer_to_agent(\"stock_analysis_agent_via_holdings\")，讓它"
        "改用使用者提供的資訊分析。\n"
        "3. get_holdings 回傳空清單或錯誤，且使用者這則訊息裡完全沒有"
        "提供任何持股資訊 -> 不要轉交，直接回覆使用者：「StockNite "
        "Agent 需要您提供更多您的持股資訊，謝謝。」，可以補一句引導"
        "使用者提供持股或跟你說明目前的股票內容。\n\n"
...
archiving_stock_agent = Agent(
    ...
    instruction=(
        ...
        "purchase_date/sold_price），然後呼叫「一次」stage_pending_"
        ...
        "purchase_date/sold_price 欄位。若某一筆回傳 already_exists "
        "錯誤，改對那一筆呼叫 update_holding；若回傳 holding_not_found "
        "錯誤，改呼叫 add_holding。一筆失敗不影響其他筆的處理，全部"
        "處理完後統整成功與失敗的結果一起回覆使用者。\n"
